    ...
    <div>
                ...
                return (
                  <tr key={l.id} className="border-b border-slate-100 last:border-0 hover:bg-slate-50/80 transition-colors">
                    <td className="px-5 py-3.5">
                      <div className="font-bold text-slate-900">{l.employeeName || 'Team Member'}</div>
                      <div className="text-xs text-slate-400 font-medium">{l.employeeRole || ''} &bull; {l.department || ''}</div>
                    </td>
                    <td className="px-5 py-3.5">
                      <div className="font-semibold text-slate-800">{l.leaveTypeDisplayName}</div>
                      {l.combinedWithType && <div className="mt-0.5 text-xs font-medium text-indigo-600">+ Combined with {l.combinedWithType}</div>}
                    </td>
                    <td className="px-5 py-3.5">
                      ...
                      <div className="text-xs text-slate-400">{l.totalDays} day{l.totalDays > 1 ? 's' : ''}</div>
                    </td>
                    <td className="max-w-[220px] px-5 py-3.5 text-sm text-slate-600">
                      {l.reason || '—'}
                      {l.documentAttached && (
                        <div className="mt-1">
                          <a href={l.documentUrl} target="_blank" rel="noreferrer" className="inline-flex items-center gap-1 text-xs font-semibold text-indigo-600 hover:underline">
                            <Icon name="document" className="h-3.5 w-3.5" strokeWidth={2} /> Medical Doc &rarr;
                          </a>
                        </div>
                      )}
                    </td>
                    <td className="px-5 py-3.5 text-right">
                      {isOwn ? (
                        <span className="rounded-full bg-slate-100 px-2.5 py-1 text-xs font-medium text-slate-500">Self-Approval Blocked</span>
                      ) : (
      ...
      {approveTarget && (
        ...
        >
          ...
          <label className="mb-1.5 block text-xs font-bold tracking-wide text-slate-700 uppercase">Approval Note / Comment (Optional)</label>
          <input
            type="text"
      ...
      {rejectTarget && (
        ...
        >
          ...
          <label className="mb-1.5 block text-xs font-bold tracking-wide text-slate-700 uppercase">Reason for Rejection *</label>
          <input
            type="text"
            required
            value={rejectComment}
      ...
      {sendBackTarget && (
        ...
        >
          ...
          <label className="mb-1.5 block text-xs font-bold tracking-wide text-slate-700 uppercase">Feedback / Requested Changes *</label>
          <input
            type="text"
            required
