    ...
    <div>
      ...
      <div className="grid grid-cols-1 gap-6 lg:grid-cols-3">
        <Card className="lg:col-span-2">
          ...
          <CardBody>
            ...
            <form onSubmit={handleSubmit} className="space-y-4.5">
              <div>
                <label className="mb-1.5 block text-xs font-bold tracking-wide text-slate-700 uppercase">Leave Type *</label>
                <select
                  required
                  value={leaveType}
                ...
                {notice && (
                  <div className={`mt-1.5 flex items-start gap-1.5 text-xs font-medium ${notice.color}`}>
                    <Icon name={leaveType === 'PAID' ? 'bell' : 'leaf'} className="mt-0.5 h-3.5 w-3.5 shrink-0" strokeWidth={2} />
              ...
              {showCombination && (
                <div>
                  <label className="mb-1.5 block text-xs font-bold tracking-wide text-slate-700 uppercase">Combine with Another Leave Type</label>
                  <select
                  ...
                  <div className="mt-1 text-xs text-slate-400">Policy Rule: Casual Leave may only be combined with WFH.</div>
                </div>
              )}
              ...
              <div className="flex flex-wrap items-center gap-2 rounded-xl border border-dashed border-slate-200 bg-slate-50/80 px-3.5 py-2.5">
                ...
                {PRESETS.map((p) => (
                  <button
                    ...
                    className="cursor-pointer rounded-lg bg-white border border-slate-200 px-2.5 py-1 text-xs font-semibold text-slate-700 shadow-2xs transition-all hover:bg-slate-50 hover:border-slate-300 active:scale-95"
                  >
                    {p.label}
                  </button>
                ))}
              ...
              <div className="flex flex-wrap items-center justify-between gap-3 rounded-xl border border-slate-100 bg-slate-50/80 px-4 py-3">
                <label className="flex items-center gap-2 text-sm font-semibold text-slate-700 cursor-pointer">
                  <input type="checkbox" checked={isHalfDay} onChange={(e) => handleHalfDayChange(e.target.checked)} className="h-4 w-4 rounded text-indigo-600 focus:ring-indigo-500" />
                  Half-Day Leave
                </label>
                {isHalfDay && (
                  <select
                ...
                <div className="text-sm font-bold text-indigo-600">Total Duration: {totalDays} Day{totalDays > 1 ? 's' : ''}</div>
              </div>

              {showDocGroup && (
                <div className="rounded-xl border border-amber-200 bg-amber-50/80 p-4 shadow-2xs">
                  <label className="font-bold text-sm text-amber-900">Medical Document Required *</label>
                  <p className="mt-0.5 mb-2.5 text-xs text-amber-700">
                  ...
                  <label className="flex items-center gap-2 text-xs font-semibold text-amber-900 cursor-pointer">
                    <input type="checkbox" checked={docAttached} onChange={(e) => setDocAttached(e.target.checked)} className="h-4 w-4 rounded text-amber-600 focus:ring-amber-500" />
                  ...
                  <input
                    ...
                    className="mt-2.5 w-full rounded-lg border border-amber-300 bg-white px-3 py-2 text-xs text-slate-800 focus:outline-none focus:ring-2 focus:ring-amber-400/40"
                  />
                </div>
              )}

              <div>
                <label className="mb-1.5 block text-xs font-bold tracking-wide text-slate-700 uppercase">Reason for Leave</label>
                <textarea
                  rows={3}
                  value={reason}
        ...
        <Card>
          ...
          <CardBody className="space-y-2.5 text-sm leading-relaxed text-slate-600">
            <p className="flex items-start gap-2">
              <span className="text-indigo-500 font-bold">•</span>
              <span><strong>Notice:</strong> Apply before the leave start date.</span>
            </p>
            <p className="flex items-start gap-2">
              <span className="text-indigo-500 font-bold">•</span>
              <span><strong>Paid Leave:</strong> Requires &gt; 2 days notice (start date must be 3+ days out).</span>
            </p>
            <p className="flex items-start gap-2">
              <span className="text-indigo-500 font-bold">•</span>
              <span><strong>Sick Leave &gt; 2 days:</strong> Medical documentation is mandatory.</span>
            </p>
            <p className="flex items-start gap-2">
              <span className="text-indigo-500 font-bold">•</span>
              <span><strong>Casual Leave:</strong> Can only be combined with WFH.</span>
            </p>
            <p className="flex items-start gap-2">
              <span className="text-indigo-500 font-bold">•</span>
              <span><strong>Weekly Cutoff:</strong> Casual/WFH requests must be submitted by end of the current week.</span>
            </p>
            <p className="flex items-start gap-2">
              <span className="text-indigo-500 font-bold">•</span>
              <span><strong>Monthly Cutoff:</strong> Sick/Paid/LOP submitted on or before the 25th.</span>
            </p>
            <div className="mt-3 rounded-xl border border-indigo-100 bg-indigo-50/70 p-3 text-xs text-indigo-800 leading-relaxed font-medium">
